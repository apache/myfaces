...
{
    private static Logger log = Logger.getLogger(DiscoverNamesInAlternateManagedProperties.class.getName());
    ...
    public ResourceNameIterator findResourceNames(final String resourceName) {
        ...
        if (log.isLoggable(Level.FINE)) {
            if (mappedName == null) {
                log.fine("find: resourceName='" + resourceName + "', no mapping");
            } else {
                log.fine("find: resourceName='" + resourceName + "', lookup property '" + mappedName + "'");
            }
        }
