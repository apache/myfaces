...
public class UIRepeatTest extends AbstractJsfTestCase
{
    ...
    /**
     * UIRepeat rejects "rowIndex" as a value-expression name and null as the name.
     */
    @Test
    public void testValueExpression()
    {
        UIRepeat repeat = new UIRepeat();
        MyFacesAsserts.assertException(IllegalArgumentException.class, new TestRunner()
        {
            public void run() throws Throwable
            {
                repeat.setValueExpression("rowIndex", null);
            }
        });
        MyFacesAsserts.assertException(NullPointerException.class, new TestRunner()
        {
            public void run() throws Throwable
            {
                repeat.setValueExpression(null, null);
            }
        });
    }
    ...
    public void testStatefulRowStateIsPreservedOnRevisit()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            Assertions.assertEquals("submitted_" + i, input.getSubmittedValue(),
                    "Row " + i + ": submitted value should be restored from clientId-keyed _rowStates");
        }
        repeat.setRowIndex(-1);
    }
