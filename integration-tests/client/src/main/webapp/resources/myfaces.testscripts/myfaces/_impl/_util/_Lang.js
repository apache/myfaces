...
class Lang {
    ...
    static getMessage (key, defaultMessage /*,vararg templateParams*/) {
        ...
        let msg = Lang._installedLocale[key] || defaultMessage || key + " - undefined message";
    ...
    static initLocale (newLocale) {
        if (newLocale) {
            Lang._installedLocale = new newLocale();
            return;
        }
        let language_Variant = Lang._RT.getLanguage(Lang._RT.getGlobalConfig("locale")),
        ...
        Lang._installedLocale = new i18nHolder();
     };
    static assertType (probe, theType) {
        return Lang._RT.assertType(probe, theType);
     };
    static exists (nms, theType) {
        return Lang._RT.exists(nms, theType);
     };
    static fetchNamespace (namespace) {
        Lang._assertStr(namespace, "fetchNamespace", "namespace");
        return Lang._RT.fetchNamespace(namespace);
     };
    static reserveNamespace (namespace) {
        Lang._assertStr(namespace, "reserveNamespace", "namespace");
        return Lang._RT.reserveNamespace(namespace);
     };
    static globalEval (code) {
        Lang._assertStr(code, "globalEval", "code");
        return  Lang._RT.globalEval(code);
     };
    /**
    ...
    static byId (/*object*/ reference) {
        if (!reference) {
            throw Lang.makeException(new Error(), null, null, Lang._nameSpace, "byId", Lang.getMessage("ERR_REF_OR_ID", null, "_Lang.byId", "reference"));
        }
        return (Lang.isString(reference)) ? document.getElementById(reference) : reference;
     };
    ...
    static strToArray (/*string*/ it, /*regexp*/ splitter) {
        ...
        Lang._assertStr(it, "strToArray", "it");
        if (!splitter) {
            throw Lang.makeException(new Error(), null, null, Lang._nameSpace, "strToArray", Lang.getMessage("ERR_PARAM_STR_RE", null, "myfaces._impl._util._Lang.strToArray", "splitter"));
        }
        let retArr = it.split(splitter);
        let len = retArr.length;
        for (let cnt = 0; cnt < len; cnt++) {
            retArr[cnt] = Lang.trim(retArr[cnt]);
        }
        return retArr;
     };
    ...
    static _assertStr (it, functionName, paramName) {
        if (!Lang.isString(it)) {
            throw Lang.makeException(new Error(), null, null, Lang._nameSpace, functionName, Lang.getMessage("ERR_PARAM_STR", null, "myfaces._impl._util._Lang." + functionName, paramName));
        }
     };
    /**
     * hyperfast trim
    ...
    static mixMaps (dest, src, overwrite, blockFilter, whitelistFilter) {
        if (!dest || !src) {
            throw Lang.makeException(new Error(), null, null, Lang._nameSpace, "mixMaps", Lang.getMessage("ERR_PARAM_MIXMAPS", null, "_Lang.mixMaps"));
        }
    ...
    static contains (arr, str) {
        if (!arr || !str) {
            throw Lang.makeException(new Error(), null, null, Lang._nameSpace, "contains", Lang.getMessage("ERR_MUST_BE_PROVIDED", null, "_Lang.contains", "arr {array}", "str {string}"));
        }
        return Lang.arrIndexOf(arr, str) != -1;
     };
    ...
    static serializeChilds (xmlNode) {
        ...
        for (let cnt = 0; cnt < xmlNode.childNodes.length; cnt++) {
            buffer.push(Lang.serializeXML(xmlNode.childNodes[cnt]));
        }
        return buffer.join("");
     };
    static isXMLParseError (xmlContent) {
        ...
        return !xmlContent ||
                (Lang.exists(xmlContent, "parseError.errorCode") && xmlContent.parseError.errorCode != 0) ||
    ...
    static attr (obj, name, value) {
        ...
        try {
            ...
            throw Lang.makeException(new Error(), null, null, Lang._nameSpace, "contains", "property " + name + " not found");
        } finally {
            findAccessor = null;
            applyAttr = null;
        }
    ...
    static makeException (error, title, name, callerCls, callFunc, message) {
        ...
        error._mfInternal.caller = callerCls || Lang._nameSpace;
